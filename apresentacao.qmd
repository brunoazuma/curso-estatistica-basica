...
Dados estruturados seguem um esquema definido, com campos, tipos e relações previsíveis. A tabela de alunos é um exemplo: cada linha representa uma observação e cada coluna representa uma variável. Esse formato facilita consultas, validações e análises tabulares [@favero_manual_analise; @aws_structured_data].

## Dados não estruturados

Dados não estruturados não seguem uma tabela fixa. Exemplos incluem textos livres, imagens, áudios e vídeos. A análise costuma exigir etapas de extração de atributos, como transcrição, classificação, reconhecimento de objetos ou análise de linguagem.

## Dados semi-estruturados

Dados semiestruturados carregam alguma organização por chaves, marcas ou metadados, mas não exigem o mesmo esquema rígido de uma tabela. JSON, XML, logs e documentos com campos opcionais são exemplos. Antes da análise, normalmente é necessário normalizar estruturas aninhadas e tratar campos ausentes.

## Vantagens e desvantagens

| Forma | Vantagens | Desafios |
|---|---|---|
| Estruturados | Consulta, validação e agregação | Esquema rígido e menor flexibilidade |
| Não estruturados | Riqueza de conteúdo e contexto | Maior custo para extração de informação |
| Semiestruturados | Flexibilidade com alguma organização | Esquemas variáveis e necessidade de transformação |

# A importância da forma dos dados

A escolha do método deve acompanhar a forma dos dados: primeiro entendemos como foram coletados e organizados; depois definimos limpeza, visualização e modelo. Essa sequência reduz interpretações indevidas e torna a análise auditável [@favero_manual_analise].
